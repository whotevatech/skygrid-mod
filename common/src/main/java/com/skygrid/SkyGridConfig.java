...
public class SkyGridConfig {
    ...
    private static final List<String> DEFAULT_OVERWORLD_WHITELIST = List.of(
        ...
        // --- Fluids: deliberately excluded ---
        // Water and lava were removed from the defaults. As grid source blocks
        // they cascade through the void and eventually flood large volumes.
        // Add them back only if you want classic flowing-fluid SkyGrid:
        ...
        "minecraft:cactus",
        // sugar_cane removed with the fluids: it is only placeable next to a
        // water source, and that water is what floods the grid.
    ...
    private static final List<String> DEFAULT_NETHER_WHITELIST = List.of(
        ...
        // "minecraft:lava" removed — see the overworld list above.
        // --- Nylium ---
    ...
    /** "overworld" / "nether" / "end" — used in command feedback and logs. */
    private String label;

    public String getLabel() { return label; }

    // -------------------------------------------------------------------------
    // Mutation — used by /skygrid add|remove|weight (and later, the config GUI)
    // -------------------------------------------------------------------------
    ...
    public int getWeight(String blockId) {
        return entries.stream()
            .filter(e -> e.id().equals(blockId))
            .mapToInt(BlockEntry::weight)
            .findFirst()
            .orElse(0);
    }

    /**
     * Adds an entry, or updates its weight if already present.
     *
     * @return true if the list changed (added, or weight differed)
     */
    public boolean addOrUpdate(String blockId, int weight) {
        int w = Math.max(1, weight);
        for (int i = 0; i < entries.size(); i++) {
            if (entries.get(i).id().equals(blockId)) {
                if (entries.get(i).weight() == w) return false;
                entries.set(i, new BlockEntry(blockId, w));
                return true;
            }
        }
        entries.add(new BlockEntry(blockId, w));
        return true;
    }

    /** @return true if an entry was actually removed */
    public boolean remove(String blockId) {
        return entries.removeIf(e -> e.id().equals(blockId));
    }

    /** Writes the current state back to the file it was loaded from. */
    public void saveToDisk() {
        if (file == null) {
            SkyGridMod.LOGGER.warn("Cannot save {} config — no backing file recorded.", label);
            return;
        }
        save(this, file);
    }
